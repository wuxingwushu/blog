...
在程序当中存储最小单位是字节，一个字节可以存储八个像素点的亮灭，字位图像素点数量等于宽乘高，而不够 8bit 的向上取整当一个完整的字节存储。再将这些位图数据按照 UTF-8标识码 的大小从小到大的排列存储起来。获取位图数据就需要获取位图第一个字节的位置，也就是数据偏移量。
...
在**UTF-8编码**内并不是所有字都是我们需要的，如：法语，俄语，我们只取需要的Unicode片段。对Unicode片段也是连续的。我们只需要将转换的Unicode减取字片段的偏移值将可以得到字对应的 字体位图信息（FontInformation）数组索引。举个简单例子：只取数字和英文大写字位图的数据，0(48)~9(57) + A(65)~Z(90) 总共有 36 字体位图信息（FontInformation）数组当然也只有 36 个。当需要英文大写**S**的数据时只需要对应的Unicode值 减去 65（[A~Z]Unicode片段起始位置） 加上 10（[A~Z]片段 前有已经存储了 [0~9]10个字） 将可以得到 字体位图信息（FontInformation） 数组索引。
...
计算 UnicodeRange 的 Deviation 值
```cpp
    // 字体 Unicode范围 偏移值(也是字体的总量)
